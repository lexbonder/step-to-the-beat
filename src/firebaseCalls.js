...
export const seedToFirebase = (id, seed) => {
  const seedToSave = firebase.database().ref().child(id).child('savedSeeds').child(seed.id);
  seedToSave.set(seed)
}

export const deleteFirebaseSeed = (userId, seedId) => {
  const seedToDelete = firebase.database().ref().child(userId).child('savedSeeds').child(seedId);
  seedToDelete.remove();
}